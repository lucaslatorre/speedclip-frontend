...
export interface Subtitle {
    ...
    text: string;
    preset: 'tiktok' | 'instagram' | 'minimal';
}
...
export class ApiMockService {
    ...
    private autosave() {
        this.videos$.subscribe(v => {
            this.state.videos = v;
            this.save();
        });
        this.cuts$.subscribe(v => {
            this.state.cuts = v;
            this.save();
        });
        this.subtitles$.subscribe(v => {
            this.state.subtitles = v;
            this.save();
        });
        this.exports$.subscribe(v => {
            this.state.exports = v;
            this.save();
        });
        this.plan$.subscribe(v => {
            this.state.plan = v;
            this.save();
        });
    }
        ...
        const subs: Subtitle[] = [
            {
                ...
                text: 'Bem-vindos ao episódio 12',
                preset: 'minimal'
            },
            {
                id: uid(),
                ...
                text: 'Hoje falamos de produtividade',
                preset: 'minimal'
            },
        ];
        return {
            ...
            subtitles: subs,
            exports: [],
            plan: {tier: 'free', minutesLimit: 60, minutesUsed: 12}
        };
    }
    ...
    listVideos(): Observable<Video[]> {
        return of(null).pipe(delay(jitter()), map(() => this.videos$.value));
    }

    addVideoFromUrl(url: string): Observable<Video> {
        const v: Video = {
            id: uid(),
            title: `URL: ${url.slice(0, 24)}`,
            duration: 1200,
            createdAt: new Date().toISOString(),
            status: 'processing'
        };
        ...
        setTimeout(() => {
            this.updateVideo(v.id, {status: 'ready'});
        }, 1200);
    ...
    /* Cuts */
    listCuts(videoId: Id): Observable<Cut[]> {
        return of(null).pipe(delay(jitter()), map(() => this.cuts$.value.filter(c => c.videoId === videoId)));
    }
    ...
    duplicateCut(cutId: Id): Observable<Cut | null> {
        const base = this.cuts$.value.find(c => c.id === cutId);
    ...
    /* Subtitles */
    listSubtitles(videoId: Id): Observable<Subtitle[]> {
        return of(null).pipe(delay(jitter()), map(() => this.subtitles$.value.filter(s => s.videoId === videoId)));
    }
    ...
    /* Exports */
    listExports(videoId: Id): Observable<ExportJob[]> {
        return of(null).pipe(delay(jitter()), map(() => this.exports$.value.filter(e => e.videoId === videoId)));
    }
    ...
    enqueueExport(videoId: Id, format: ExportJob['format'], resolution: ExportJob['resolution']): Observable<ExportJob> {
        ...
        setTimeout(() => {
            this.updateExport(job.id, {status: 'processing'});
        }, 800);
        setTimeout(() => {
            this.updateExport(job.id, {status: 'done'});
        }, 2000);
    ...
    getPlan(): Observable<Plan> {
        return of(null).pipe(delay(jitter()), map(() => this.plan$.value));
    }

    setPlan(tier: Plan['tier']): Observable<Plan> {
        const next: Plan = tier === 'free' ? {
            tier: 'free',
            minutesLimit: 60,
            minutesUsed: this.plan$.value.minutesUsed
        } : {tier: 'pro', minutesLimit: 600, minutesUsed: this.plan$.value.minutesUsed};
        this.plan$.next(next);
